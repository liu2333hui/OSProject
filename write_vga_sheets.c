...
//void showChar8(char *vram, int xsize, int x, int y, char c, unsigned int* font);
...
/*	
	...
	*/
	...
	//Display the total amount of free memory that can be used in
	//a given page (page = 3, address = 100000, mem = FDE0000)
	memman_init(memman);
...
void showString(char *vram, int xsize, int x, int y, char c, unsigned char*s){
	...
	while(*s != 0x00){
		...
		//showChar8(vram, xsize, x, y, c, get_font() + (*s)*16);
...
//char get_font_data(int c, int offset);
	//write line by line
	//sprites are 8 bits by 16 bits down
	/*
	for(i = 0; i < 16; i++){
		d = get_font_data(*s, i);
		//d = *(font + i);	
